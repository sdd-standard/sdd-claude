...
- Requisitos em GEARS (Generalized EARS): `[Where <estático>] [While <dinâmico>] [When <gatilho>] the <subject> shall <behavior>`. Keywords sempre em inglês.
- Semântica GEARS: `Where` = pré-condição estática (configuração, feature flag, ambiente); `While` = pré-condição de estado mutável em runtime (ex: enquanto conta bloqueada); `When` = evento ou gatilho de entrada.
- Intent Block captura intenção (Goal/Expectation/Action/Result); GEARS formaliza comportamento observável e testável. Nunca confunda os dois.
